...
import type { IndividualIdTypeOman, OwnerRecord, PartyType } from '@/lib/types'
...
function ownerWritePayload(input: CreateOwnerInput): Record<string, unknown> {
...
export async function patchOwnerRecord(
  id: string,
  patch: Partial<Omit<OwnerRecord, 'id' | 'createdAt' | 'updatedAt'>>,
): Promise<void> {
  if (!db) throw new Error('Firestore not initialized')
  const partial: Record<string, unknown> = {}
  const keys: (keyof Omit<OwnerRecord, 'id' | 'createdAt' | 'updatedAt'>)[] = [
    'partyType',
    'nameEn',
    'nameAr',
    'email',
